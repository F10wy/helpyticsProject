...
function seedData() {
  ...
  const notes = load(STORAGE_KEYS.notifications, []);
  if (notes.length === 0) {
  ...
  return `
    <article class="request-card fade-in">
      ...
      <div class="list-item" style="padding-bottom:0;border-bottom:0;">
        ...
        <a class="btn btn-secondary" href="request-detail.html?id=${request.id}">Open details</a>
      </div>
    </article>
  `;
}
...
function renderDashboard() {
  ...
  if (qs("[data-dashboard-name]")) qs("[data-dashboard-name]").textContent = currentUser.name;
  if (qs("[data-stat-trust]")) qs("[data-stat-trust]").textContent = `${currentUser.trustScore}%`;
  if (qs("[data-stat-helping]")) qs("[data-stat-helping]").textContent = requests.filter((request) => request.helperIds.includes(currentUser.id)).length;
  if (qs("[data-stat-open]")) qs("[data-stat-open]").textContent = requests.filter((request) => request.status === "Open").length;
  if (qs("[data-stat-insights]")) qs("[data-stat-insights]").textContent = `${requests.filter((request) => request.category === "Career").length} trends`;
  if (qs("[data-dashboard-requests]")) qs("[data-dashboard-requests]").innerHTML = requests.slice(0, 3).map((request) => requestCard(request, users)).join("");
  if (qs("[data-dashboard-notes]")) qs("[data-dashboard-notes]").innerHTML = notifications.slice(0, 3).map((note) => `
  ...
  `).join("");
  if (qs("[data-dashboard-ai]")) qs("[data-dashboard-ai]").innerHTML = `
  ...
  <header className="topbar">
    <div className="container nav">
      ...
      <nav className="nav-links">
        <a data-nav="dashboard" href="/dashboard">
          Dashboard
        </a>
      ...
      <div className="nav-actions">
        <a className="pill" href="/notifications">
          Notifications
        </a>
        <a className="btn btn-primary" href="/ai-center">
          Open AI Center
        </a>
      </div>
    </div>
  ...
  <main className="container">
    <section className="page-hero">
      <div className="panel">
        ...
        <h1 style={{ fontSize: "clamp(2.3rem, 4vw, 4.2rem)" }}>
          Welcome back, <span data-dashboard-name="" />.
        </h1>
        <p>
    ...
    <section className="dashboard-grid section">
      <div className="stack">
        ...
        <div className="stack" data-dashboard-requests="" />
      </div>
      <div className="stack">
        <div className="panel">
          <p className="section-kicker">AI insights</p>
          <h3>Suggested actions for you</h3>
          <div data-dashboard-ai="" />
        </div>
        <div className="panel">
          <p className="section-kicker">Notifications</p>
          <h3>Latest updates</h3>
          <div className="notif-list" data-dashboard-notes="" />
        </div>
      </div>
    </section>
  </main>
